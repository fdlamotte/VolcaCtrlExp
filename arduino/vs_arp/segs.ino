...
#define DIGIT_OFF  LOW
  ...
  // changed so that display doesn't bother other parts of code
  if ((millis() - lastTime) < 10)  // only update every 5ms
    return;
  ...
  for(int digit = 4 ; digit > 0 ; digit--) {
    ...
    // we don't delay but call display more often (every ms)
  ...
  //while( (millis() - beginTime) < 1) ; //Wait for 1ms to pass before we paint the display again
